...
| 5   | Extract koanf into configload sub-module                                                          | **Moot** — `configload` was deleted entirely (commit `e3e710c`); KoanfLoader now lives in `v4` (`koanf_loader.go`). There is no sub-package left to extract. |
