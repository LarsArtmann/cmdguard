...
- `samber-do-auditlog` is consumed from the Go module proxy (`v0.8.1`). The sibling repo at `../samber-do-auditlog` is for local dev only — a `replace` directive works for local builds but is **ignored by downstream consumers** (replace directives in a library's go.mod only affect the module's own build/CI).
