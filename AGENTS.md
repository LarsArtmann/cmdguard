...
│   │   ├── result.go             # Result[T] and Validated[T] sum types
...
18. **Audit log integration** — `WithAuditLog[T](plugin)` wires `samber-do-auditlog` into the DI injector; `cli.AuditLog()`/`cli.AuditLogReport()` for programmatic access; `AuditLogServiceByName`/`AuditLogFailedServices` query helpers; `ExportAuditLog` supports 11 formats (html, json, ndjson, csv, tsv, mermaid, dot, d2, plantuml, tree, htmltree). No built-in subcommand — consumers export via their own flag/env pattern (e.g. `DO_AUDITLOG_ENABLED` + `AUDIT_LOG_FORMAT`)
19. **Plugin system** — `Plugin` interface bundles custom type handlers + validators; `RegisterPlugin()` applies globally, `WithPlugin[T]()` / `FlagRegistry.RegisterPlugin()` apply per-instance
20. **Sum types** — `Result[T]` (Ok/Err) and `Validated[T]` (Valid/Invalid) provide explicit error handling without panics
21. **Nested config structs** — `ParseFlagTags` recurses into nested structs; `FieldTag.Index` tracks the reflect path for flattened flag registration
22. **Docs generation** — `cli.GenerateDocs(w)` writes markdown documentation for the full command tree to any `io.Writer`
...
- `WithConfigFile[T](paths...)` loads config BEFORE flag registration; config values become the new tag defaults, so flags/env still override them
...
- **`WithGracefulShutdown[T]()`** enables DI service shutdown on SIGINT/SIGTERM (implies `WithSignalHandling`). Services implementing `do.ShutdownerWithError` shut down in reverse invocation order. `WithSignalHandling` only cancels context and does NOT trigger DI shutdown.
...
- **Strict** — `WithStrictValidation[T]()` requires `WithShort` on all commands; enforced at `AddCommand` time
- **Draconian** — `WithDraconianValidation[T]()` is strict + requires `WithExample` on leaf commands; parent commands are exempt
...
- **Spinner non-TTY** — `SpinnerMiddleware` auto-skips when `os.Stderr` is not a terminal; override with `SpinnerConfig{Writer: ...}`
- **Telemetry context** — `TelemetryMiddleware` starts a span but cannot propagate the new context to the handler (`next func() error` signature); child spans must use the original context
...
- **Prompt tag** — `prompt:"Question?"` + `WithPromptOnMissing` enables interactive prompting for missing flags. Bool → `huh.NewConfirm`, enum (with `values` tag) → `huh.NewSelect`, else → `huh.NewInput`.
