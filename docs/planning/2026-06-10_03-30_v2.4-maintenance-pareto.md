...
This v2.4 plan targets `pkg/cmdguard/v2/` paths that no longer exist; cmdguard is now `github.com/larsartmann/cmdguard/v3` (mono-repo + 5 sub-modules, tagged 2026-07-07). The P0 bugs (MergeConfigs zero-value loss, Enum.UnmarshalText bypass, setStringField panic, ExitError nil panic) and the panic-elimination work were carried into v3 and resolved there. Retained as a historical snapshot of the v2.4 maintenance queue.
