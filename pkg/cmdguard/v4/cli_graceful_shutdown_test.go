...
func newGracefulShutdownTestCLI(t *testing.T) *v4.CLI[testCLIConfig] {
	t.Helper()
...
func TestGracefulShutdown_CallsShutdownerOnScope(t *testing.T) {
	...
	t.Run("shutdown is called on service implementing ShutdownerWithError", func(t *testing.T) {
		...
		err := v4.Provide(cli.Scope(), func(i do.Injector) (*gracefulShutdownService, error) {
			return svc, nil
		})
		if err != nil {
		...
		_, err = v4.Invoke[*gracefulShutdownService](cli.Scope())
		if err != nil {
