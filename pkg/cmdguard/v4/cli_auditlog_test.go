...
func TestAuditLogConvenienceHelpers(t *testing.T) {
	...
	t.Run("AuditLogServiceByName returns nil when not enabled", func(t *testing.T) {
		...
		svc := v4.AuditLogServiceByName(cli, "anything")
		if svc != nil {
	...
	t.Run("AuditLogServiceByName returns service when enabled", func(t *testing.T) {
		...
		svc := v4.AuditLogServiceByName(cli, name)
		if svc == nil {
...
func TestParseAuditLogFormat(t *testing.T) {
	...
	tests := []struct {
		input   string
		want    v4.AuditLogFormat
		wantErr bool
	}{
		{"", v4.AuditLogFormatHTML, false},
		{"html", v4.AuditLogFormatHTML, false},
		{"htmltree", v4.AuditLogFormatHTMLTree, false},
		{"json", v4.AuditLogFormatJSON, false},
		{"ndjson", v4.AuditLogFormatNDJSON, false},
		{"mermaid", v4.AuditLogFormatMermaid, false},
		{"csv", v4.AuditLogFormatCSV, false},
		{"tsv", v4.AuditLogFormatTSV, false},
		{"dot", v4.AuditLogFormatDOT, false},
		{"d2", v4.AuditLogFormatD2, false},
		{"plantuml", v4.AuditLogFormatPlantUML, false},
		{"tree", v4.AuditLogFormatTree, false},
		{"xml", "", true},
		{"HTML", "", true},
	}
	...
	for _, tt := range tests {
		t.Run(tt.input, func(t *testing.T) {
			...
			got, err := v4.ParseAuditLogFormat(tt.input)
			if tt.wantErr {
				if err == nil {
					t.Fatal("expected error")
				}
				if !errors.Is(err, v4.ErrUnsupportedAuditLogFormat) {
...
func TestExportAuditLog(t *testing.T) {
	...
	t.Run("noop when audit log not enabled", func(t *testing.T) {
		...
		err := v4.ExportAuditLog(cli, v4.AuditLogExportConfig{
			Format: v4.AuditLogFormatHTML,
		})
		if err != nil {
	...
	formats := []v4.AuditLogFormat{
		v4.AuditLogFormatJSON,
		v4.AuditLogFormatNDJSON,
		v4.AuditLogFormatMermaid,
		v4.AuditLogFormatCSV,
		v4.AuditLogFormatTSV,
		v4.AuditLogFormatDOT,
		v4.AuditLogFormatD2,
		v4.AuditLogFormatPlantUML,
		v4.AuditLogFormatTree,
		v4.AuditLogFormatHTMLTree,
	}
